...
def validate_completion_file_content(content):
    """
    Validate DEGAP_PROCESS_COMPLETED file content format
    Expected format: "DEGAP processing completion time: YYYY-MM-DD HH:MM:SS"
    ...
    expected_prefix = "DEGAP processing completion time: "

    # Check if content starts with expected prefix
    if not content.startswith(expected_prefix):
        return False

    # Extract timestamp part
    timestamp_part = content[len(expected_prefix):]

    # Basic format validation: YYYY-MM-DD HH:MM:SS (19 characters)
    ...
    try:
        ...
        # Check if all date components are digits
        ...
        # Check if all time components are digits
        if not (hour.isdigit() and minute.isdigit() and second.isdigit()):
            return False

        # Basic range validation
        if not (0 <= int(month) <= 12 and 0 <= int(day) <= 31 and
...
def generate_job_script(reads_path, output_dir, optional_params=None, flag='all', mode='gapfiller'):
    ...
    """
    # Create GapJobOutput directory
    degap_output = os.path.join(output_dir, 'GapJobOutput')
...
def generate_batch_lsf_commands(output_dir, batch_count=10, cores=3, queue="normal", flag='all'):
    ...
    with open(batch_script_path, 'w') as f:
        ...
        if flag == 'all':
            ...
            f.write(f"DEGAP_OUTPUT=\"{os.path.abspath(os.path.join(output_dir, 'GapJobOutput'))}\"\n\n")
...
def extract_filled_gap_info(output_dir, genome_file, flag='all'):
    ...
    degap_output_dir = os.path.join(output_dir, 'GapJobOutput')
    if not os.path.exists(degap_output_dir):
        print(f"Warning: Cannot find GapJobOutput directory: {degap_output_dir}")
...
def main(args, stats):
    ...
    if args.post_process:
        # Post-processing stage - process gap filling results
        ...
        # Step 1: Read gap.log file to get gap location information
        gap_log_path = os.path.join(output_dir_abs, 'gap.log')
        if not os.path.exists(gap_log_path):
            print(f"Error: gap.log file not found at {gap_log_path}")
            _exit_program(1)

        print(f"Reading gap information from: {gap_log_path}")
        gaps_info = []
        with open(gap_log_path, 'r') as f:
            ...
            for line in f:
                ...
                if len(parts) >= 7:
                    ...
                    gaps_info.append(gap_info)

        print(f"Found {len(gaps_info)} gaps to process")

        # Step 2: Load original genome sequences
        print("Loading original genome sequences...")
        genome_sequences = {}
        for record in SeqIO.parse(genome_abs, "fasta"):
            genome_sequences[record.id] = str(record.seq)

        # Step 3: Process each gap according to flag mode
        degap_output_dir = os.path.join(output_dir_abs, 'GapJobOutput')
        filled_gaps = []
        unfilled_gaps = []

        for gap in gaps_info:
            print(f"\nProcessing gap {gap['chromosome']}.{gap['gap_id']}...")
            gap_result = process_single_gap(gap, degap_output_dir, flag, genome_sequences)

            if gap_result['filled']:
                filled_gaps.append(gap_result)
                print(f"  -> Filled: {gap_result['fill_type']}, Extension length: {gap_result['extension_length']}")
            else:
                unfilled_gaps.append(gap_result)
                print(f"  -> Not filled")

        # Step 4: Save results and generate final genome
        print(f"\n=== Processing Summary ===")
        print(f"Total gaps: {len(gaps_info)}")
        print(f"Filled gaps: {len(filled_gaps)}")
        ...
        with open(filled_genome_path, 'w') as f:
            for chrom_id, seq in genome_sequences.items():
                f.write(f">{chrom_id}\n{seq}\n")

        print(f"Filled genome saved to: {filled_genome_path}")

        # Generate summary log
        summary_path = os.path.join(output_dir_abs, 'post_processing_summary.log')
        generate_post_processing_summary(summary_path, filled_gaps, unfilled_gaps)
        ...
        try:
            # Import and call the visualization module
            import AutoGapfillerVisualizer

            # Check for required files in the main output directory
            required_files = [
                ('gap.log', 'Original gap information'),
            ...
            if not available_files:
                print("Error: No required files found for visualization")
                raise FileNotFoundError("Required visualization input files not found")

            # Generate visualization using the main output directory directly
            visualization_output = os.path.join(output_dir_abs, 'AutoGapfiller_Results.html')

            # Create argument namespace for visualization
            viz_args = argparse.Namespace()
            viz_args.input_dir = output_dir_abs  # Use main output directory directly
            viz_args.output = visualization_output
            viz_args.genome = genome_abs if os.path.exists(genome_abs) else None

            print(f"Generating visualization with {len(available_files)} input files...")
            print(f"Input directory: {output_dir_abs}")
            print(f"Output file: {visualization_output}")

            # Call visualization function
            AutoGapfillerVisualizer.main(viz_args)

            if os.path.exists(visualization_output):
                print(f"✓ Visualization report generated successfully: {visualization_output}")
                print(f"  Open the HTML file in a web browser to view the results.")
            else:
                print("✗ Visualization file was not created")

        except ImportError as e:
            print(f"Warning: AutoGapfillerVisualizer module not found: {str(e)}")
            print("Skipping visualization generation. Please ensure AutoGapfillerVisualizer.py is in the same directory.")
        except Exception as e:
            print(f"Warning: Error generating visualization: {str(e)}")
            print("Post-processing completed successfully, but visualization generation failed.")
            print("You can manually generate the visualization later using:")
            print(f"python AutoGapfillerVisualizer.py --input-dir {output_dir_abs} --output {os.path.join(output_dir_abs, 'AutoGapfiller_Results.html')}")

        # If called from monitoring, force exit
    ...
    optional_params = {}
    for param in ['--kmer_size', '--kmer_num', '--kmer_length', '-j', '--remove', '--edge', '--filterDepth', '--MaximumExtensionLength']:
    ...
    if jobs_submitted:
        # INITIAL CHECK: Check if DEGAP_PROCESS_COMPLETED file already exists with valid content
        completion_flag = os.path.join(output_dir_abs, 'DEGAP_PROCESS_COMPLETED')
        if os.path.exists(completion_flag):
            print(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] DEGAP_PROCESS_COMPLETED file already exists!")
            try:
                with open(completion_flag, 'r') as f:
                    content = f.read().strip()
                print(f"Completion file content: {content}")

                # Validate content format using strict validation function
                if validate_completion_file_content(content):
                    print("Valid completion file content detected.")
                    print("DEGAP processing has already been completed. Main LSF job will exit immediately.")
                    print("\n=== DEGAP processing completed, script will exit ===")
                    _exit_program(0)  # Force exit immediately
                else:
                    print("Warning: Completion file exists but content format is invalid.")
                    print("Expected format: 'DEGAP processing completion time: YYYY-MM-DD HH:MM:SS'")
                    print("Continuing with normal monitoring...")

            except Exception as e:
                print(f"Could not read completion file content: {e}")
                print("Continuing with normal monitoring...")
        ...
        print("Maximum monitoring time: 48 hours")
        print("Press Ctrl+C to interrupt monitoring at any time, post-processing can be run manually later")

        try:
            max_monitoring_hours = 48
            monitoring_start_time = time.time()
            check_count = 0

            while True:
                check_count += 1
                elapsed_hours = (time.time() - monitoring_start_time) / 3600

                # HIGHEST PRIORITY CHECK: Check if DEGAP_PROCESS_COMPLETED file exists with valid content
                completion_flag = os.path.join(output_dir_abs, 'DEGAP_PROCESS_COMPLETED')
                if os.path.exists(completion_flag):
                    print(f"\n[{time.strftime('%Y-%m-%d %H:%M:%S')}] DEGAP_PROCESS_COMPLETED file detected!")
                    try:
                        with open(completion_flag, 'r') as f:
                            content = f.read().strip()
                        print(f"Completion file content: {content}")
                        ...
                        if validate_completion_file_content(content):
                            print("Valid completion file content detected.")
                            print("DEGAP processing has been completed. Main LSF job will exit immediately.")
                            print("\n=== DEGAP processing completed, script will exit ===")
                            _exit_program(0)  # Force exit immediately
                        else:
                            print("Warning: Completion file exists but content format is invalid.")
                ...
                if elapsed_hours > max_monitoring_hours:
                    print(f"\nMaximum monitoring time ({max_monitoring_hours} hours) exceeded.")
                    print("Stopping monitoring. You can run post-processing manually:")
                ...
                try:
                    ...
                    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Check #{check_count} - Completed: {completed}/{total} ({completed/total*100:.1f}%) - Elapsed: {elapsed_hours:.1f}h")
                except Exception as e:
                    print(f"Error checking gap completion status: {e}")
...
def process_single_gap(gap, degap_output_dir, flag, genome_sequences):
    """
    Process a single gap according to the flag mode and return result information

    Parameters:
    gap: Gap information dictionary
    degap_output_dir: GapJobOutput directory path
    flag: Processing flag ('left', 'right', or 'all')
    genome_sequences: Dictionary of chromosome sequences (will be modified in place)

    Returns:
    Dictionary with gap processing results
    """
    gap_name = f"{gap['chromosome']}.{gap['gap_id']}"

    if flag in ['left', 'right']:
        # Single direction processing
        return process_single_direction(gap, degap_output_dir, flag, genome_sequences)
    elif flag == 'all':
        # All directions processing - check both left and right
        return process_all_directions(gap, degap_output_dir, genome_sequences)
    else:
        return {
            'gap': gap,
            'filled': False,
            'fill_type': 'Unknown flag',
            'extension_length': 0,
            'sequence': ''
        }

def process_single_direction(gap, degap_output_dir, direction, genome_sequences):
    """Process a single direction for a gap"""
    gap_name = f"{gap['chromosome']}.{gap['gap_id']}"
    result_dir = os.path.join(degap_output_dir, f"{gap_name}.{direction}")
    ...
    if not os.path.exists(result_dir):
        return {
            'gap': gap,
            'filled': False,
            'fill_type': 'Result directory not found',
            'extension_length': 0,
            'sequence': ''
        }

    # Read process.log to determine the outcome
    process_log_path = os.path.join(result_dir, 'process.log')
    if not os.path.exists(process_log_path):
        return {
            'gap': gap,
            'filled': False,
            'fill_type': 'Process log not found',
            'extension_length': 0,
            'sequence': ''
        }

    # Analyze process.log content
    with open(process_log_path, 'r') as f:
        log_content = f.read()

    # Check for different outcomes
    if "No ExtensionReads or ExtensionContig Found" in log_content and "Endloop!" in log_content:
        if "Decided to use direct connection result" in log_content:
            # Result 2: Direct connection
            return process_direct_connection(gap, result_dir, genome_sequences)
        else:
            # Result 1: Not filled
            return {
                'gap': gap,
                'filled': False,
                'fill_type': 'No extension found',
                'extension_length': 0,
                'sequence': ''
            }
    elif "Final ExtensionSequence:" in log_content and "Final EXtendFile:" in log_content:
        # Result 3: Extension connection
        return process_extension_connection(gap, result_dir, genome_sequences)
    else:
        # Unknown outcome
        return {
            'gap': gap,
            'filled': False,
            'fill_type': 'Unknown outcome',
            'extension_length': 0,
            'sequence': ''
        }

def process_all_directions(gap, degap_output_dir, genome_sequences):
    """Process both left and right directions for a gap with termination logic"""
    gap_name = f"{gap['chromosome']}.{gap['gap_id']}"
    left_dir = os.path.join(degap_output_dir, f"{gap_name}.left")
    right_dir = os.path.join(degap_output_dir, f"{gap_name}.right")

    # Check for termination files first
    left_terminated_file = os.path.join(left_dir, "TERMINATED_BY_RIGHT_SUCCESS")
    right_terminated_file = os.path.join(right_dir, "TERMINATED_BY_LEFT_SUCCESS")

    if os.path.exists(right_terminated_file):
        # Right was successful, process left directory
        return process_single_direction(gap, degap_output_dir, 'left', genome_sequences)
    elif os.path.exists(left_terminated_file):
        # Left was successful, process right directory
        return process_single_direction(gap, degap_output_dir, 'right', genome_sequences)
    else:
        # No termination files, process left first (as per requirement)
        left_result = process_single_direction(gap, degap_output_dir, 'left', genome_sequences)
        if left_result['filled']:
            return left_result
        else:
            # If left failed, try right
            return process_single_direction(gap, degap_output_dir, 'right', genome_sequences)

def process_direct_connection(gap, result_dir, genome_sequences):
    """Process direct connection result (Result 2)"""
    # Find direct.final.fa file
    direct_files = [f for f in os.listdir(result_dir) if f.endswith('.direct.final.fa')]
    if not direct_files:
        return {
            'gap': gap,
            'filled': False,
            'fill_type': 'Direct connection file not found',
            'extension_length': 0,
            'sequence': ''
        }

    direct_file_path = os.path.join(result_dir, direct_files[0])

    # Read the sequence from direct.final.fa
    try:
        sequences = []
        for record in SeqIO.parse(direct_file_path, "fasta"):
        ...
        if not sequences:
            return {
                'gap': gap,
                'filled': False,
                'fill_type': 'Empty direct connection file',
                'extension_length': 0,
                'sequence': ''
            }

        # Use the first sequence (or concatenate if multiple)
        filled_sequence = ''.join(sequences)

        # Replace the gap in the genome sequence
        replace_gap_in_genome(gap, filled_sequence, genome_sequences)

        # Calculate extension length (negative for direct connection)
        seed_length = gap['left_len']  # Assuming left_len == right_len
        extension_length = len(filled_sequence) - 2 * seed_length

        return {
            'gap': gap,
            'filled': True,
            'fill_type': 'Direct Connection',
        ...
        return {
            'gap': gap,
            'filled': False,
            'fill_type': f'Error reading direct connection file: {str(e)}',
            'extension_length': 0,
            'sequence': ''
        }

def process_extension_connection(gap, result_dir, genome_sequences):
    """Process extension connection result (Result 3)"""
    # Find .final.fa file (not .direct.final.fa)
    final_files = [f for f in os.listdir(result_dir) if f.endswith('.final.fa') and not f.endswith('.direct.final.fa')]
    if not final_files:
        return {
            'gap': gap,
            'filled': False,
            'fill_type': 'Extension final file not found',
            'extension_length': 0,
            'sequence': ''
        }

    final_file_path = os.path.join(result_dir, final_files[0])

    # Read the sequence from .final.fa
    try:
        sequences = []
        for record in SeqIO.parse(final_file_path, "fasta"):
            sequences.append(str(record.seq))
        ...
        if not sequences:
            return {
                'gap': gap,
                'filled': False,
                'fill_type': 'Empty extension final file',
                'extension_length': 0,
                'sequence': ''
            }
        ...
        # Calculate extension length (positive for extension connection)
        seed_length = gap['left_len']  # Assuming left_len == right_len
        extension_length = len(filled_sequence) - 2 * seed_length
        ...
        return {
            'gap': gap,
            'filled': True,
            'fill_type': 'Extension Connection',
            'extension_length': extension_length,
            'sequence': filled_sequence
        }
        ...
        return {
            'gap': gap,
            'filled': False,
            'fill_type': f'Error reading extension final file: {str(e)}',
            'extension_length': 0,
            'sequence': ''
        }

def replace_gap_in_genome(gap, filled_sequence, genome_sequences):
    """Replace gap region in the genome sequence with filled sequence"""
    chromosome = gap['chromosome']
    if chromosome not in genome_sequences:
        print(f"Warning: Chromosome {chromosome} not found in genome sequences")
        return

    # Calculate the actual region to replace (left_seed + gap + right_seed)
    start_pos = gap['start'] - gap['left_len']  # Start of left seed
    end_pos = gap['end'] + gap['right_len']     # End of right seed

    # Ensure positions are within bounds
    if start_pos < 0:
        start_pos = 0
    if end_pos >= len(genome_sequences[chromosome]):
        end_pos = len(genome_sequences[chromosome]) - 1

    # Replace the region
    original_seq = genome_sequences[chromosome]
    new_seq = original_seq[:start_pos] + filled_sequence + original_seq[end_pos + 1:]
    genome_sequences[chromosome] = new_seq

    print(f"  Replaced region {start_pos}-{end_pos} in {chromosome} with {len(filled_sequence)}bp sequence")

def generate_post_processing_summary(summary_path, filled_gaps, unfilled_gaps):
    """Generate a summary log of post-processing results"""
    with open(summary_path, 'w') as f:
        f.write("=== DEGAP Post-Processing Summary ===\n\n")
        f.write(f"Total gaps processed: {len(filled_gaps) + len(unfilled_gaps)}\n")
        f.write(f"Filled gaps: {len(filled_gaps)}\n")
        f.write(f"Unfilled gaps: {len(unfilled_gaps)}\n\n")

        if filled_gaps:
            f.write("=== Filled Gaps ===\n")
            f.write("Chromosome\tGap_ID\tFill_Type\tExtension_Length\tOriginal_Gap_Length\n")
            for result in filled_gaps:
                gap = result['gap']
                f.write(f"{gap['chromosome']}\t{gap['gap_id']}\t{result['fill_type']}\t{result['extension_length']}\t{gap['gap_len']}\n")
            f.write("\n")

        if unfilled_gaps:
            f.write("=== Unfilled Gaps ===\n")
            f.write("Chromosome\tGap_ID\tReason\tOriginal_Gap_Length\n")
            for result in unfilled_gaps:
                gap = result['gap']
                f.write(f"{gap['chromosome']}\t{gap['gap_id']}\t{result['fill_type']}\t{gap['gap_len']}\n")
...
# Basic command:
# python AutoGapfiller.py --reads path/to/reads --genome path/to/genome --mode gapfiller --flag left -o path/to/outputdir
# You can change default parameter values through -kn|--kmer_num  -ks|--kmer_size  -kl|--kmer_length -j --remove --edge --filterDepth --MaximumExtensionLength
