...
def getoptions():
	...
	if args.mode == "gapfiller":
		...
		# Check files required for telseeker mode
		if args.seqleft:
			if os.path.exists(args.seqleft) and os.path.getsize(args.seqleft) != 0:
				seqleft = args.seqleft
			else:
			...
			print("seqleft parameter is required for telseeker mode")
			sys.exit()
		...
		if args.seqright:
			...
			print("seqright parameter is required for telseeker mode")
			sys.exit()
		...
		print("You should use gapfiller, ctglinker, or telseeker!")
		sys.exit()
...
if parameter[0] == "gapfiller":
    ...
    if resume_round is not None:
        ...
        telseeker.resume_round = resume_round
    else:
        # Normal startup for telseeker
        TelSeeker(parameter, kparameters)
print ('welldone')
